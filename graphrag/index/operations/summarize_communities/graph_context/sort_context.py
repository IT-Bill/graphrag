...
def sort_context(
    ...
    for edge in edges:
        ...
        if max_completion_tokens and num_tokens(new_context_string) > max_completion_tokens:
            break
...
def parallel_sort_context_batch(community_df, max_completion_tokens, parallel=False):
    ...
    if parallel:
        ...
        with ThreadPoolExecutor(max_workers=None) as executor:
            context_strings = list(
                executor.map(
                    lambda x: sort_context(x, max_completion_tokens=max_completion_tokens),
        ...
        community_df[schemas.CONTEXT_STRING] = community_df[schemas.ALL_CONTEXT].apply(
            lambda context_list: sort_context(context_list, max_completion_tokens=max_completion_tokens)
        )
    ...
    community_df[schemas.CONTEXT_EXCEED_FLAG] = (
        community_df[schemas.CONTEXT_SIZE] > max_completion_tokens
    )
