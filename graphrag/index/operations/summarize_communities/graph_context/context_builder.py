...
def build_local_context(
    ...
    for level in progress_iterable(levels, callbacks.progress, len(levels)):
        communities_at_level_df = _prepare_reports_at_level(
            nodes, edges, claims, level, max_completion_tokens
        )
...
def build_level_context(
    ...
    if report_df is None or report_df.empty:
        invalid_context_df.loc[:, schemas.CONTEXT_STRING] = _sort_and_trim_context(
            invalid_context_df, max_completion_tokens
        )
    ...
    community_df = _get_community_df(
        level, invalid_context_df, sub_context_df, community_hierarchy_df, max_completion_tokens
    )
    ...
    remaining_df.loc[:, schemas.CONTEXT_STRING] = _sort_and_trim_context(
        remaining_df, max_completion_tokens
    )
...
def _sort_and_trim_context(df: pd.DataFrame, max_completion_tokens: int) -> pd.Series:
    """Sort and trim context to fit the limit."""
    series = cast("pd.Series", df[schemas.ALL_CONTEXT])
    return transform_series(series, lambda x: sort_context(x, max_completion_tokens=max_completion_tokens))


def _build_mixed_context(df: pd.DataFrame, max_completion_tokens: int) -> pd.Series:
    """Sort and trim context to fit the limit."""
    series = cast("pd.Series", df[schemas.ALL_CONTEXT])
    return transform_series(
        series, lambda x: build_mixed_context(x, max_completion_tokens=max_completion_tokens)
    )
...
def _get_community_df(
    ...
    community_df[schemas.CONTEXT_STRING] = _build_mixed_context(
        community_df, max_completion_tokens
    )
