...
class LocalSearchMixedContext(LocalContextBuilder):
        ...
        **kwargs: dict[str, Any],
        ...
        if conversation_history:
            ...
            if conversation_history_context.strip() != "":
                ...
                max_completion_tokens = max_completion_tokens - num_tokens(
                    conversation_history_context, self.token_encoder
                )

        # build community context
        community_tokens = max(int(max_completion_tokens * community_prop), 0)
        community_context, community_context_data = self._build_community_context(
            selected_entities=selected_entities,
            max_completion_tokens=community_tokens,
        ...
        local_tokens = max(int(max_completion_tokens * local_prop), 0)
        local_context, local_context_data = self._build_local_context(
            selected_entities=selected_entities,
            max_completion_tokens=local_tokens,
        ...
        text_unit_tokens = max(int(max_completion_tokens * text_unit_prop), 0)
        text_unit_context, text_unit_context_data = self._build_text_unit_context(
            selected_entities=selected_entities,
            max_completion_tokens=text_unit_tokens,
            return_candidate_context=return_candidate_context,
        )
    ...
    def _build_community_context(
        ...
        """Add community data to the context window until it hits the max_completion_tokens limit."""
    ...
    def _build_text_unit_context(
        ...
        """Rank matching text units and add them to the context window until it hits the max_completion_tokens limit."""
